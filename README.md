...
(train, val) と test についてはホールドアウト検証の形をとっています。

test データの設定をユーザーの判断に委ねられるようにするためです。

train と val については一般的なk分割交差検証(アンサンブル学習)を実装しています。

## 主要な機能追加・修正の履歴

（メモ：バージョン名は履歴のほかindex.htmlのヘッダーのみに付している）

* 2026/8/12 目的変数の標準化、欠損値の処理、損失関数(LOSS)の保存機能を追加しました。
* 2026/9/4 補完時のリークを防ぐ措置、誤差の表示、乱数シードの表示、UIシンプル化、説明の充実化を実施しました。
* 2026/9/14 CSV読み込みの選択肢追加、自動最適化機能の追加を実施しました。
* 2026/9/15 補完部分を修正し、Version 1.0.0 としました！
